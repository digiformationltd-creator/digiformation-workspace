...
//   sold → strike_off → address_default → auth_missing → ready_to_sell → active
// ---------------------------------------------------------------------------
...
export function groupByPrimaryCategory(list: Company[]): GroupedCompanies {
  ...
  for (const c of list) {
    // Defensive: DB may still return legacy "ad01_processing" primary_category
    // for a short window; fall back to raw derivation so those rows land in a
    // valid bucket instead of crashing the group lookup.
    const cat = RULES.getPrimaryCategory(c);
    const bucket = groups[cat] ?? groups[RULES.derivePrimaryCategoryFromRaw(c)];
    bucket.push(c);
  }
  return groups;
}
